            ...
            <div className="p-6 space-y-8">
                <div>
                    <h2 className="text-xl font-black text-emerald-500 tracking-tight flex items-center gap-2 cursor-pointer" onClick={() => nav('/home')}>
                        SmartQueue <span className="text-sm bg-slate-800 text-slate-300 px-2 py-0.5 rounded-md">STAFF</span>
                    </h2>
                </div>
                ...
                <nav className="space-y-2">
                    {links.map(l => (
                        <NavLink
                            key={l.path}
                            to={l.path}
                            end={l.path === '/kitchen'} // End matching so Live Queue doesn't always highlight
                            className={({ isActive }) =>
