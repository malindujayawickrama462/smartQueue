...
export default function StudentOrder() {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-100 px-4 py-10">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2">
                        {AVAILABLE_TAGS.map(tag => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition ${filterTag === tag ? 'bg-emerald-500 text-slate-950' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
                            >
                                {tag}
                            </button>
                        ))}
                                ...
                                <>
                                    ...
                                    <div className="border-t border-slate-700 pt-3 space-y-2">
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="font-bold text-slate-100">Rs. {originalTotalPrice.toFixed(2)}</span>
                                        </div>
                                        {pointsDiscount > 0 && (
                                            <div className="flex justify-between text-sm text-emerald-400">
                                                <span>Points Discount:</span>
                                                <span className="font-bold">-Rs. {pointsDiscount.toFixed(2)}</span>
                                            </div>
                                        )}
                                        <div className="flex justify-between text-lg font-bold pt-2 border-t border-slate-600">
                                            <span>Total:</span>
                                            <span className="text-emerald-400">Rs. {finalPrice.toFixed(2)}</span>
                                        </div>
                                    </div>
                                    ...
                                    {walletInfo && (
                                        <div className="p-3 rounded-lg bg-slate-800/50 border border-slate-700/50 space-y-2">
                                            ...
                                            <p className="text-sm text-slate-100">Balance: <span className="font-bold text-emerald-400">{walletInfo.loyaltyPoints || 0} pts</span></p>
                                            <input
                                                type="number"
                                                min="0"
                                                max={Math.min(walletInfo.loyaltyPoints || 0, originalTotalPrice)}
                                                value={redeemingPoints}
                                                onChange={(e) => setRedeemingPoints(Math.min(Number(e.target.value), walletInfo.loyaltyPoints || 0))}
                                                className="w-full bg-slate-900/50 border border-slate-700 rounded-lg px-3 py-2 text-slate-200 focus:outline-none text-sm"
                                    ...
                                    <div className="flex gap-2 pt-2">
                                        ...
                                        {walletInfo && (
                                            <button
                                                ...
                                                disabled={loading}
                                                className="flex-1 py-2 px-3 rounded-lg bg-purple-500/10 text-purple-400 border border-purple-500/20 hover:bg-purple-500 hover:text-slate-950 transition font-bold text-sm disabled:opacity-50"
                                            >
                                                👛 Wallet
                                            </button>
                                        )}
            ...
            {showCheckout && <CheckoutModal onClose={() => setShowCheckout(false)} cart={cart} totalPrice={finalPrice} onPaymentSuccess={(transactionId) => processOrderPlacement('Card', transactionId)} />}
        </div>
    );
}